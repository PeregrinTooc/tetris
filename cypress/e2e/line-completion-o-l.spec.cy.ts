...
describe("Line completion with O and L pieces", () => {
    ...
    it("should fill 4 rows with O-pieces and complete 2 lines with an L-piece on the right", () => {
        ...
        cy.get('#game-board [data-tetromino-id="6"].block').should("exist").then($blocks => {
            ...
            expect(bottomAreaBlocks.length, `L-tetromino blocks should be in bottom area after line completion, got positions: ${positions}`).to.be.greaterThan(0);

            // At least some blocks should be quite low (at least one in bottom 3 rows)
            const veryBottomBlocks = positions.filter(y => y >= 18);
            expect(veryBottomBlocks.length, `At least one L-tetromino block should be in very bottom area (y>=18), got positions: ${positions}`).to.be.greaterThan(0);
        });
    });
});
