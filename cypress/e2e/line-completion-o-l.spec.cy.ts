...
describe("Line completion with O and L pieces", () => {
	...
	it("should fill 4 rows with O-pieces and complete 2 lines with an L-piece on the right", () => {
		...
		getBlocksByTetrominoId("6", "#game-board").should("exist");

		// Use grid position helper to get rows
		getBlockGridPositions("6").then((positions) => {
			const rows = positions.map((p) => p.row);
			cy.log(`L-tetromino grid rows: ${rows.join(", ")}`);
			...
			const bottomAreaBlocks = rows.filter((y) => y >= 16);
			expect(
				bottomAreaBlocks.length,
				`L-tetromino blocks should be in bottom area after line completion, got rows: ${rows}`
			).to.be.greaterThan(0);

			// At least some blocks should be quite low (at least one in bottom 3 rows)
			const veryBottomBlocks = rows.filter((y) => y >= 18);
			expect(
				veryBottomBlocks.length,
				`At least one L-tetromino block should be in very bottom area (y>=18), got rows: ${rows}`
			).to.be.greaterThan(0);
		});
	});
});
